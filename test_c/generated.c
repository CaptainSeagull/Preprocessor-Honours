...
/* Recreated structs. */
typedef struct Position Position; struct Position {
    float x;
    float y;
};

typedef struct Test Test; struct Test {
    int sixteen[16];
    float float_four[4];
...
typedef struct FooOne FooOne; struct FooOne {
    int one;
    int two;
    int three;
};
            ...
            default: {
                switch(member->type) {
                    case meta_type_Position: {
                        if(member->is_ptr) {
                            bytes_written = serialize_struct_(**(char **)member_ptr, Position, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        } else {
                            bytes_written = serialize_struct_(*(char *)member_ptr, Position, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        }
                    } break;
                    ...
                    case meta_type_Test: {
                        if(member->is_ptr) {
                            bytes_written = serialize_struct_(**(char **)member_ptr, Test, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        } else {
                            bytes_written = serialize_struct_(*(char *)member_ptr, Test, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        }
                    } break;
                    ...
                    case meta_type_FooOne: {
                        if(member->is_ptr) {
                            bytes_written = serialize_struct_(**(char **)member_ptr, FooOne, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        } else {
                            bytes_written = serialize_struct_(*(char *)member_ptr, FooOne, member->name, indent, buffer, buf_size - bytes_written, bytes_written);
                        }
                    } break;
