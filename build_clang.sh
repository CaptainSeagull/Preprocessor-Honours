...
if [ "$RELEASE"=="true" ]; then
    ...
    if [ "$GTEST"=="true" ]; then
        clang++-3.8 $WARNING_LEVEL "preprocessor/preprocessor.cpp" "preprocessor/google_test/gtest-all.cc" $CPP_VERSION $DISABLED -o $OUTPUT_NAME -DERROR_LOGGING=1 -DRUN_TESTS=1 -DINTERNAL=1 -DMEM_CHECK=0 $DEFINES $WARNINGS -g -ldl -pthread
    else
        clang++-3.8 $WARNING_LEVEL "preprocessor/preprocessor.cpp" $CPP_VERSION $DISABLED -o $OUTPUT_NAME -DERROR_LOGGING=1 -DRUN_TESTS=0 -DINTERNAL=1 -DMEM_CHECK=0 $DEFINES $WARNINGS -g -ldl
    fi
fi
